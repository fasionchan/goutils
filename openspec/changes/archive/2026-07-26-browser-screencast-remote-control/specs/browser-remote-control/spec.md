...
远程控制连接 SHALL 使用 WebSocket。服务端 MUST 将 screencast 图像以 BinaryMessage 推送（载荷为图像原始字节，不含 JSON 包装）。所有控制、会话与事件消息 MUST 使用 TextMessage，且正文为 JSON envelope。

#### Scenario: 推送 screencast 帧

- **WHEN** screencast 产生一帧图像数据
- **THEN** 服务端以 BinaryMessage 将该帧字节写入 WebSocket，不经过 JSON 编码
